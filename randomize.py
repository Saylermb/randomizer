...
\x1e\x1fSymbols0020!"#$%&\'()*+,-./0123456789:;<=>?
040@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_Alphabet0060`a
...
ˣˤ˥˦˧˨˩˪˫ˬ˭ˮ˯˰˱˲˳˴˵˶˷˸˹˺˻˼˽˾˿ 0300̛̖̗̘̙̜̝̞̟̀́̂̃̄̅̆̇̈̉̊̋̌̍̎̏̐̑̒̓̔̕̚ 0320̴̵̶̷̸̡̢̧̨̠̣̤̥̦̩̪̫̬̭̮̯̰̱̲̳
̹̺̻̼̽̾̿ 0340͇͈͉͍͎̀́͂̓̈́͆͊͋͌ͅ͏͓͔͕͖͙͚͐͑͒͗͛͘͜͟͝͞ 0360ͣͤͥͦͧͨͩͪͫͬͭͮͯ͢͠͡ͰͱͲͳʹ͵Ͷͷ\u0378\u0379ͺͻͼͽ;Ϳ 0380
...
՞՟ 0560ՠաբգդեզէըթժիլխծկհձղճմյնշոչպջռսվտ 0580րցւփքօֆևֈ։֊
...
ПЮО1GрTScаghxкvzUNЦгЖУФ0qАdпЭ бoяfyН]уi
...
def random_datatime(a: datetime, b: datetime):
    a, b = int(a.timestamp()), int(b.timestamp())
    return datetime.fromtimestamp(randint(a, b))


def random_text_unicode(size: int = 1, random_size=False):
    if random_size:
        size = randint(0, size)
    result = ''
    for s in range(size):
...
def random_text(size: int = 1, random_size=False):
    if random_size:
        size = randint(0, size)
    result = ''
    for s in range(size):
